...
class CharacterController implements ICharacterController {
    ...
    moveBy(x: number, y: number, next: Action): void {
        ...
        moveTween.onComplete.add(() => {
            ...
            var collision = this.game.collisionController.checkCollisions(this.sprite);
            if (collision) {
    ...
    onCollision(data: ColliderData, next: Action) {
        switch (data.name) {
            case "diamond":
                this.diamondCount++;
